...
interface BillEntry {
  id?: string;
  hotel_id: string;
  utility_type: "electricity" | "gas" | "water" | "chp";
  filename: string;
  bill_period?: string;
  total_amount: number;
  consumption: number;
...
interface BillsListModalProps {
  bills: BillEntry[];
  onClose: () => void;
  utilityType?: "electricity" | "gas" | "water" | "chp" | "all";
  month?: string;
  year?: string;
  hotelId?: string;
}
...
export default function BillsListModal({
  ...
  const filteredBills = bills.filter((bill) => {
    ...
    if (month || year) {
      ...
      const startDate =
        bill.summary?.billing_period_start ||
      ...
      if (startDate && endDate) {
        ...
        if (month && year) {
          ...
          const targetMonthStart = new Date(parseInt(year, 10), monthNum, 1);
          const targetMonthEnd = new Date(parseInt(year, 10), monthNum + 1, 0); // Last day of month
          ...
          const targetYearStart = new Date(parseInt(year, 10), 0, 1);
          const targetYearEnd = new Date(parseInt(year, 10), 11, 31);
  ...
  const getS3PdfUrl = (bill: BillEntry, forViewing = false) => {
    const billYear = bill.summary?.bill_date ? bill.summary.bill_date.substring(0, 4) : year || "2024";

    const baseUrl = `${process.env.NEXT_PUBLIC_API_URL}/utilities/bill-pdf/${bill.hotel_id}/${bill.utility_type}/${billYear}/${encodeURIComponent(
      bill.filename
    )}`;
  ...
  const downloadPdf = async (bill: BillEntry) => {
    ...
    try {
      const pdfUrl = getS3PdfUrl(bill, false);
  ...
  const openPdf = async (bill: BillEntry) => {
    ...
    try {
      const pdfUrl = getS3PdfUrl(bill, true);
          ...
          {filteredBills.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredBills.map((bill, index) => {
                ...
                return (
                  <div key={billId || index} className="border border-slate-200 rounded-xl p-6 hover:bg-slate-50 transition-colors">
                    <div className="flex items-start justify-between">
                      {/* Left side - Bill info */}
                      <div className="flex items-start space-x-4 flex-1">
                        <div className="flex-shrink-0 mt-1">{getUtilityIcon(bill.utility_type)}</div>

                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-3 mb-2">
                            <h4 className="font-semibold text-slate-900 capitalize">{bill.utility_type} Bill</h4>
                          ...
                          <div className="text-xs text-slate-500 font-mono">File: {bill.filename}</div>
                        </div>
                      </div>
          ...
          <div className="border-t border-slate-200 px-6 py-4 bg-slate-50">
            <div className="flex items-center justify-between text-sm text-slate-600">
              ...
              <div className="flex items-center space-x-4">
                <span>
                  €{formatCurrency(filteredBills.reduce((sum, bill) => sum + (bill.summary?.total_cost || bill.total_amount), 0)).replace("€", "")} total
                </span>

                <span>
                  {filteredBills
                    .reduce((sum, bill) => sum + (bill.summary?.total_kwh || bill.summary?.consumption_kwh || bill.consumption), 0)
                    .toLocaleString()}{" "}
                  {filteredBills[0]?.consumption_unit || "kWh"}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-[95vw] w-full max-h-[95vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-4 border-b bg-slate-50">
              ...
              <button onClick={() => setViewingDetails(null)} className="p-2 hover:bg-gray-200 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-1/2 border-r flex flex-col">
                ...
                <div className="flex-1 bg-gray-100 relative">
                  <iframe src={getS3PdfUrl(viewingDetails, true)} className="w-full h-full border-0" title="PDF Viewer" />
                </div>
              </div>
              ...
              <div className="w-1/2 flex flex-col">
                <div className="p-3 bg-gray-50 border-b">
                  ...
                  <p className="text-sm text-gray-600">
                    Status:{" "}
                    <span className={`px-2 py-0.5 rounded text-xs ${getBillStatus(viewingDetails).color}`}>
                    ...
                    <div className="bg-blue-50 rounded-lg p-4 space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-blue-800">
                          {(() => {
                            const consumption =
                              viewingDetails.summary?.total_kwh ||
                              viewingDetails.summary?.consumption_kwh ||
                              viewingDetails.consumption;
                            const unit = viewingDetails.consumption_unit || viewingDetails.raw_data?.consumptionDetails?.consumptionUnit || "kWh";
            ...
            <div className="p-4 border-t bg-slate-50 flex justify-between items-center">
              ...
              <div className="flex space-x-2">
                ...
                <button onClick={() => setViewingDetails(null)} className="px-4 py-2 bg-slate-600 text-white rounded-lg hover:bg-slate-700">
                  Close
                </button>
              </div>
            </div>
