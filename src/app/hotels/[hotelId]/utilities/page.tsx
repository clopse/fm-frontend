...
export default function UtilitiesDashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const res = await fetch(`http://localhost:8000/api/utilities/${hotelId}/${year}`);
      const data = await res.json();

      console.log("🔌 Electricity data from backend:", data.electricity);
      console.log("🔥 Gas data from backend:", data.gas);
  ...
  useEffect(() => {
    async function fetchAllHotelData() {
      const promises = hotelOptions.map(async (id) => {
        const res = await fetch(`http://localhost:8000/api/utilities/${id}/${year}`);
        const data = await res.json();
  ...
  const getElectricityData = () => {
    const mapped = electricity.map((e) => ({
      ...e,
      value:
        viewMode === "eur"
          ? e.total_eur
    ...
    console.log("⚡ Mapped electricity data:", mapped);
    return mapped;
  };

  const getGasData = () => {
    const mapped = gas.map((g) => ({
      ...g,
      value:
        viewMode === "eur"
          ? g.total_eur
    ...
    console.log("🔥 Mapped gas data:", mapped);
    return mapped;
  };
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.controls}>
        <h2>{hotelId?.toUpperCase()} Utilities Dashboard</h2>
        <div>
